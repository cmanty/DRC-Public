...
format: 
  revealjs:
    ...
    title-slide-attributes:
      data-background-image: "Microsoft Teams Backdrops_DRC-Deloitte_Light Gray Gradient.jpg"
...
The number of unemployed people in DFW is `r format(topline_indicators_BLS$unemp_count[topline_indicators_BLS$year == endyear], scientific = FALSE, big.mark = ",")` in `r endyear[1]`.
...
emp_by_metro <- get_n_series_table(
  ...
  select(year, period, "Phoenix" = "SMU04380600000000001", "Los Angeles" = "SMU06310800000000001", "San Francisco" = "SMU06418600000000001", "Washington DC" = "SMU11479000000000001", "Miami" = "SMU12331000000000001", "Atlanta" = "SMU13120600000000001", "Chicago" = "SMU17169800000000001", "Boston" = "SMU25716500000000001", "NYC" = "SMU36356200000000001", "Philadelphia" = "SMU42379800000000001", "Dallas" = "SMU48191000000000001", "Houston" = "SMU48264200000000001") %>%
  mutate(period = str_extract(period, "[1-9]+[0-9]*"))
...
emp_metro_graph_fiveyear <- ggplot(emp_metro_five, aes(reorder(Metro, -value), y=value, fill = Metro)) + 
    geom_bar( position = "dodge", stat= "identity") +
    scale_fill_viridis(discrete = TRUE, option = "A", labels = "Count") +
    scale_y_continuous(limits = c(-50, 600), breaks = seq(0, 600, by = 100)) +
    ...
    labs(title = "5-Year Net Change 2019-2024") +
  geom_text(aes(label = format(value, digits = 2), vjust = ifelse(value > 0, -1, 2)), size = 3.5, position = position_dodge(width = 0.8), hjust = 0.5)
...
emp_metro_covid <- emp_by_metro %>%
  ...
  filter(date %in% c("2019-12-01", paste0(endyear, "-", ifelse(max(as.numeric(period[year == endyear])) > 9, max(as.numeric(period[year == endyear])), paste0("0", max(as.numeric(period[year == endyear])))), "-", "01"))) %>%
  select(-c(period, date)) %>%
  pivot_longer(names_to = "Metro", cols = Phoenix:Houston, values_to = "employment") %>%
  pivot_wider(names_from = year, values_from = employment, names_prefix = "emp") %>%
  mutate(empchange_ct = get(paste0("emp", endyear))-emp2019) %>%
  mutate(empchange_pct = 100*(empchange_ct/emp2019)) %>%
  pivot_longer(names_to = "variable", values_to = "value", cols = empchange_ct:empchange_pct) %>%
...
emp_metro_graph_covid <- ggplot(emp_metro_covid, aes(reorder(Metro, -value), y=value, fill = Metro)) + 
    ...
    theme(axis.text.x = element_text(angle = 45, size = 12),
          ...
          axis.title = element_blank(),
          legend.position = "none") +
    labs(title = "Percent Change since Pre-Pandemic Peak") +
  geom_text(aes(label = format(value, digits = 2), vjust = ifelse(value > 0, -1, 2)), size = 3.5, position = position_dodge(width = 0.8), hjust = 0.5)
...
emp_ind_compare_yoy <- emp_ind_dfw_yoy %>%
 ...
 emp_industry_graph_yoy <- ggplot(emp_ind_compare_yoy, aes(reorder(Industry, -value), y=value, fill = geography)) +
     geom_bar( position = "dodge", stat= "identity") +
     scale_fill_viridis(discrete = TRUE, option = "A") +
     ...
     labs(title = "Industry Employment", y = "Percent Change in Employment", x = "Industry", fill = "Metric") +
   geom_text(aes(label = format(value, digits = 2), vjust = ifelse(value > 0, -1, 2)), size = 3, position = position_dodge(width = 0.8), hjust = 0.5)
...
```{r emp_industry, fig.asp=0.8, dpi=300}
...
######## ^^^^^^^^^^^^ STILL NEED TO CHANGE FOR INDUSTRY (NEEDS CHECK) ^^^^^^^^^^ ###########
 ...
 emp_industry_graph_fiveyear <- ggplot(emp_ind_dfw_fiveyear, aes(reorder(Industry, -value), y=value, fill = Industry)) + 
    geom_bar( position = "dodge", stat= "identity") +
    scale_fill_viridis(discrete = TRUE, option = "A", labels = "Count") +
  scale_y_continuous(limits = c(0, 160), breaks = seq(0, 150, by = 25)) +
    theme(axis.text.x = element_text(angle = 45, size = 12),
          axis.text.y = element_text(size = 12),
          plot.title = element_text(hjust = 0.5),
          plot.subtitle = element_text(hjust = 0.5, size = 6),
          ...
          #axis.title = element_blank(),
          legend.position = "none") +
    labs(title = "5-Year Net Change 2019-2024", y ="Change in Employment (Thousands)", x = "Industry") +
...
emp_industry_graph_covid <- ggplot(emp_ind_dfw_covid, aes(reorder(Industry, -value), y=value, fill = Industry)) + 
    geom_bar( position = "dodge", stat= "identity") +
    scale_fill_viridis(discrete = TRUE, option = "A", labels = "Count") +
  scale_y_continuous(limits = c(0, 22.5), breaks = seq(0, 20, by = 5)) +
    theme(axis.text.x = element_text(angle = 45, size = 12),
          axis.text.y = element_text(size = 12),
          plot.title = element_text(hjust = 0.5),
          plot.subtitle = element_text(hjust = 0.5, size = 6),
          ...
          #axis.title = element_blank(),
          legend.position = "none") +
    labs(title = "Percent Change since Pre-Pandemic Peak", y = "Percent Change in Employment", x = "Industry") +
  geom_text(aes(label = format(value, digits = 2)), size = 3.5, position = position_dodge(width = 0.8), vjust = -1, hjust = 0.5)
...
emp_region_fiveyear <- emp_by_region %>%
  ...
  mutate(rest_tx_change = rest_TX - lag(rest_TX, 12),
...
emp_region_fiveyear_graph <- ggplot(emp_region_fiveyear, aes(x = date, y=Employment, color = Region)) +
  ...
  geom_text_repel(aes(label = ifelse(date == max(date[Region == "dfw_change"]),
                                     paste0(format(Employment, digits = 2),
                                            " ",
                                            case_when(Region == "rest_tx_change" ~ "Rest of Texas",
...
```{r emp_region, fig.asp=0.5, dpi=300}
...
unemp_area_fiveyear_graph <- ggplot(unemp_area_fiveyear, aes(x = date, y=Unemployment, color = Region)) +
  ...
  theme(axis.text.x = element_text(angle = 45),
          ...
          axis.text.x.bottom = element_text(vjust=0.6),
          axis.title = element_blank()) +
  labs(title = "YoY Net Change in Unemployment 2019-2024") +
  geom_text_repel(aes(label = ifelse(date == max(date[Region == "dfw"]),
                                     paste0(format(Unemployment, digits = 2),
                                            "%",
                                            " ",
                                            case_when(Region == "us" ~ "US",
                                                      Region == "texas" ~ "Texas",
                                                      Region == "dfw" ~ "DFW",
                                                      TRUE ~ NA_character_)),
                                     NA)),
...
```{r emp_div2, fig.asp=0.6, dpi=300}
...
```{r echo=FALSE}
 ...
 cpitables <- get_n_series_table(c("CUUR0000SA0",
                                    "CUUR0000SA0E",
                                    "CUUR0000SA0L1E",
                                    "CUUR0000SAF1",
                                    "CUUR0370SA0",
...
cpitable_rate <- cpitables %>%
  ...
  mutate(date = paste(period, year, sep = "-")) %>%
  mutate(date = my(date, locale = Sys.getlocale("LC_TIME"))) %>%
  mutate(us.all.items.change = 100*(us.all.items - lag(us.all.items, 12))/lag(us.all.items, 12),
...
cpi_rates_oneyear <- cpitable_rate %>%
  ...
  mutate(category = str_extract(variable, "(?<=\\.).*(?=\\.change)"),
...
cpi_oneyear_graph <- ggplot(cpi_rates_oneyear, aes(x = category, y = value, fill = region)) +
  geom_bar(position = "dodge", stat = "identity") +
  scale_fill_viridis(discrete = TRUE, labels = c("DFW", "Large Cities", "US", "West South Central")) +
  scale_x_discrete(labels = c("All Items", "Core", "Energy", "Food")) +
  scale_y_continuous(limits = c(-11, 5), breaks = seq(-10, 4, by = 2)) +
  ...
  labs(title = "YoY Percent Change in Inflation", fill = "Region")
...
inflationgraphrate <- ggplot(cpi_rate_dfw, aes(x = date, y = value, color = variable)) +
  ...
  theme(axis.text.x = element_text(angle = 45, size = 12),
          ...
          axis.text.x.bottom = element_text(vjust=0.6),
          axis.title = element_blank()) +
          labs(title = "YoY Percent Change in Inflation", color = "Metric") +
  geom_text_repel(aes(label = ifelse(date == max(date),
                                     paste0(format(value, digits = 2),
                                            " ",
                                            case_when(variable == "dfw.all.items.change" ~ "All CPI Items",
...
::::: container
::: column
```{r inflation_1, fig.asp=0.5, dpi=300}
...
::::: container
::: column
```{r inflation_2, fig.asp=0.5, dpi=300}
